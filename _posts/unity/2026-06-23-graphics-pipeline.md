...
화면에 보이는 모든 3D 오브젝트는 **버텍스(Vertex, 점)** 에서 시작한다.
...
- **Face Culling** : 카메라를 등진 뒷면 폴리곤 제거. GPU는 카메라에서 봤을 때 삼각형의 버텍스 연결 순서가 반시계 방향(CCW)이면 앞면, 시계 방향(CW)이면 뒷면으로 판단한다.
  ```
    1
   / \
  3 - 2
  1→2→3 순서로 이으면 → CCW(앞면)
  1→3→2 순서로 이으면 → CW(뒷면)
  ```
